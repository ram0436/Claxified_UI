  ...
  applyForVacancy() {
    ...
    if (
      ...
      if (mobile.length === 13) {
        ...
        this.userService.applyForVacancy(vacancyData).subscribe(
          (response) => {
          ...
          (error) => {
            this.showNotification(
              "An error occur while submitting your application."
            );
          }
        );
      } else {
      ...
      this.showNotification(
        "Please fill in all the details and select a resume file."
      );
    }
  }
