...
export class AddPostComponent {
  ...
  constructor(private commercialService: CommercialService, private commonService: CommonService, private snackBar: MatSnackBar, private route: ActivatedRoute, private AdminDashboardService: AdminDashboardService,
  ...
  selectImage(event: any): void {
    ...
    this.commercialService.uploadCommercialServiceImages(formData).subscribe((data: any) => {
      this.progress = false;
  ...
  savePetPost(payload: any) {
    if (this.validatePostForm(payload))
      ...
      this.commercialService.saveCommercialServicePost(payload).subscribe(data => {
        this.showNotification("Post added succesfully");
        this.router.navigateByUrl('/post-menu');
      });
  }
  addSpecificPayload(commonPayload: any): any {
    var imageList: { commercialServicesId: number; imageId: string; imageURL: any; }[] = [];
    this.cardsCount.forEach(imageURL => {
      if (imageURL != "")
        imageList.push({ "commercialServicesId": 0, "imageId": "100", "imageURL": imageURL });
    });
    var payload = Object.assign({}, commonPayload, {
      commercialServiceImagesList: imageList,
    });
    return payload;
  }
  getUserData() {
  ...
  validatePostForm(payload: any): boolean {
    ...
    else if (payload.commercialServiceImagesList.length <= 0)
