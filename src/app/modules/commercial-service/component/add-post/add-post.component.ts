...
export class AddPostComponent {
    ...
    this.route.queryParams.subscribe((params) => {
      ...
      if (this.mode != undefined) {
        ...
        this.commercialService
          .getCommercialServicePostByGuid(guid)
          .subscribe((res: any) => {
            this.commonPayload = res[0];
            res[0].commercialServiceImagesList.forEach(
              (image: any, index: any) => {
  ...
  selectImage(event: any): void {
    ...
    this.commercialService
      .uploadCommercialServiceImages(formData)
      .subscribe((data: any) => {
        this.progress = false;
  ...
  postAdd() {
    ...
    if (payload.id) this.updateCommercialServicePost(payload);
    else this.saveCommercialServicePost(payload);
  }
  ...
  saveCommercialServicePost(payload: any) {
    if (this.validatePostForm(payload)) {
      ...
      this.commercialService
        .saveCommercialServicePost(payload)
        .subscribe((data) => {
  ...
  updateCommercialServicePost(payload: any) {
    if (this.validatePostForm(payload)) {
      ...
      this.commercialService
        .updateCommercialServicePost(payload)
        .subscribe((data) => {
