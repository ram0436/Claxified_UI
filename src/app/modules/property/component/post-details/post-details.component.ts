  ...
  getPropertyPost(guid: any) {
    this.propertyService.getPropertyPostById(guid).subscribe((data: any) => {
      ...
      this.houeseType = this.houseTypes.filter(
        (houseType) => houseType.id == this.postDetails.houseType
      );
      if (this.houeseType.length > 0)
        this.postDetails.houseType = this.houeseType[0].label;
      this.furnishing = this.furnishingStatus.filter(
        (furnishing) => furnishing.id == this.postDetails.furnishingStatus
      );
      if (this.furnishing.length > 0)
        this.postDetails.furnishingStatus = this.furnishing[0].label;
      this.listed_By = this.listedBy.filter(
        (listedBy) => listedBy.id == this.postDetails.listedBy
      );
      if (this.listed_By.length > 0)
        this.postDetails.listedBy = this.listed_By[0].label;
      this.facingType = this.facingTypes.filter(
        (facingType) => facingType.id == this.postDetails.facingType
      );
      if (this.facingType.length > 0)
        this.postDetails.facingType = this.facingType[0].label;
      this.serviceType = this.serviceTypes.filter(
        (serviceType) => serviceType.id == this.postDetails.serviceType
      );
      if (this.serviceType.length > 0)
        this.postDetails.serviceType = this.serviceType[0].label;
      this.pgType = this.pgTypes.filter(
        (pgType) => pgType.id == this.postDetails.pgType
      );
      if (this.pgType.length > 0)
        this.postDetails.pgType = this.pgType[0].label;
      this.construction_status = this.constructionStatus.filter(
        (status) => status.id == this.postDetails.constructionStatus
      );
