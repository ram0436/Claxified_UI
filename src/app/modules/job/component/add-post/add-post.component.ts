...
export class AddPostComponent implements AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.salaryPeriods = this.salaryPeriods.slice(
      this.salaryPeriods.length / 2
    );
    this.positionTypes = this.positionTypes.slice(
      this.positionTypes.length / 2
    );
    this.getUserData();
  ...
  postAdd() {
    ...
    if (payload.id) this.updateJobPost(payload);
    else this.saveJobPost(payload);
  }
  ...
  setSubCategory() {
    this.commonService
      .getSubCategoryByCategoryId(this.commonPayload.categoryId)
      .subscribe((data: any) => {
  ...
  addSpecificPayload(commonPayload: any): any {
    ...
    var payload = Object.assign({}, commonPayload, {
      ...
      salaryFrom:
        this.jobData.salaryFrom == null ? 0 : Number(this.jobData.salaryFrom),
      salaryTo:
        this.jobData.salaryTo == null ? 0 : Number(this.jobData.salaryTo),
    });
    return payload;
  }
  getUserData() {
  ...
  validatePostForm(payload: any): boolean {
    ...
    else if (payload.title == "") this.showNotification("Title is required");
  ...
  selectSalaryPeriod(salaryPeriod: any) {
    ...
    if (salaryPeriod.selected) this.jobData.salaryPeriodType = salaryPeriod.id;
    else this.jobData.salaryPeriodType = 0;
  }
  selectPositionType(positionType: any) {
    positionType.selected = !positionType.selected;
    if (positionType.selected) this.jobData.positionType = positionType.id;
    else this.jobData.positionType = 0;
  }
