...
export class AddPostComponent {
  ...
  constructor(private furnitureService: FurnitureService, private commonService: CommonService, private snackBar: MatSnackBar, private route: ActivatedRoute,
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if(mode !=undefined){
        ...
        this.furnitureService.getFurniturePostByGuid(guid).subscribe((res:any)=>{
          ...
          res[0].furnitureImageList.forEach((image:any,index:any)=>{
  ...
  selectImage(event: any): void {
    ...
    this.furnitureService.uploadFurnitureImages(formData).subscribe((data: any) => {
      this.progress = false;
  ...
  postAdd() {
    ...
    if(payload.id)
    this.updateFurniturePost(payload);
  else
  ...
  validatePostForm(payload: any): boolean {
    ...
    else if (payload.furnitureImageList.length <= 0)
  ...
  updateFurniturePost(payload: any) {
    if (this.validatePostForm(payload))
      this.furnitureService.updateFurniturePost(payload).subscribe(data => {
