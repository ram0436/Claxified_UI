...
export class AdminDashboardComponent implements OnInit {
  ...
  getCardImageURL(card: any): string {
    if (card.gadgetImageList && card.gadgetImageList[0]?.imageURL) {
      ...
      card.commercialServiceImageList &&
      card.commercialServiceImageList[0]?.imageURL
    ) {
  ...
  getPostDetailsLink(card: any): any[] {
    const mainCategory = this.mainCategories.find(
      (mainCategory: any) => mainCategory.id == card.categoryId
    );
  ...
  editCard(data: any) {
    ...
    if (mainCategory != null) {
      ...
      switch (mainCategory.categoryName) {
        case "Gadgets": {
          this.router.navigateByUrl(
            `/Gadgets/add-post?main=Gadgets&sub=Mobiles&mode=edit&fromAdmin=true&categoryId=${categoryId}&tableRefGuid=${tableRefGuid}`
          );
          break;
        }
        case "Vehicles": {
          this.router.navigateByUrl(
            `/Vehicles/add-post?main=Vehicles&sub=Cars&mode=edit&fromAdmin=true&categoryId=${categoryId}&tableRefGuid=${tableRefGuid}`
          );
          break;
        }
        case "Books": {
        ...
        case "Commercial Services": {
          this.router.navigateByUrl(
            `/Commercial Services/add-post?main=Commercial Services&sub=Finance & Management&mode=edit&fromAdmin=true&categoryId=${categoryId}&tableRefGuid=${tableRefGuid}`
          );
          break;
        }
        case "Properties": {
          this.router.navigateByUrl(
            `/Properties/add-post?main=Properties&sub=For Sale: Houses%20%26%20Apartments&mode=edit&fromAdmin=true&categoryId=${categoryId}&tableRefGuid=${tableRefGuid}`
          );
          break;
        }
        case "Jobs": {
        ...
        case "Fashion": {
          this.router.navigateByUrl(
            `/Fashion/add-post?main=Fashion&sub=Men&mode=edit&fromAdmin=true&categoryId=${categoryId}&tableRefGuid=${tableRefGuid}`
          );
          break;
        }
      }
