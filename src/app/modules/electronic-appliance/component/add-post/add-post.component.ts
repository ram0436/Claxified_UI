...
export class AddPostComponent {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if(this.mode !=undefined){
        ...
        this.electronicApplianceService.getElectronicAppliancePostByGuid(guid).subscribe((res:any)=>{
          this.commonPayload = res[0];
          res[0].electronicApplianceImageList.forEach((image:any,index:any)=>{
  ...
  postAdd() {
    ...
    if(payload.id)
      this.updateElectronicAppliancePost(payload);
    else
  ...
  updateElectronicAppliancePost(payload: any) {
    if (this.validatePostForm(payload))
      this.electronicApplianceService.updateElectronicAppliancePost(payload).subscribe(data => {
