...
export class AddBusinessOfferingComponent implements OnInit {
  ...
  async onSave(): Promise<void> {
    ...
    // DEBUG: log the exact payload being sent to saveBusinessOffering
    console.log('[AddOffering] saveBusinessOffering payload:', offeringPayload);
    console.log(
      '[AddOffering] saveBusinessOffering payload (JSON):',
      JSON.stringify(offeringPayload, null, 2),
    );

    this.businessService.saveBusinessOffering(offeringPayload).subscribe(
      (savedOffering) => {
        console.log(
          '[AddOffering] saveBusinessOffering response:',
          savedOffering,
        );
      ...
      (err) => {
        // DEBUG: log the full error so we can see status/body, not just "internal server error"
        console.error('[AddOffering] saveBusinessOffering FAILED');
        console.error('[AddOffering] status:', err?.status, err?.statusText);
        console.error('[AddOffering] error body:', err?.error);
        console.error('[AddOffering] full error object:', err);
        console.error(
          '[AddOffering] payload that caused the failure:',
          offeringPayload,
        );
  ...
  private saveTypeSpecificDetail(
    ...
    if (offeringType === OfferingType.Course) {
      ...
      // DEBUG
      console.log('[AddOffering] saveOfferingCourse payload:', coursePayload);
      console.log(
        '[AddOffering] saveOfferingCourse payload (JSON):',
        JSON.stringify(coursePayload, null, 2),
      );

      this.businessService.saveOfferingCourse(coursePayload).subscribe(
        (res) => {
          console.log('[AddOffering] saveOfferingCourse response:', res);
          this.finishSave();
        },
        (err) => {
          console.error('[AddOffering] saveOfferingCourse FAILED');
          console.error('[AddOffering] status:', err?.status, err?.statusText);
          console.error('[AddOffering] error body:', err?.error);
          console.error('[AddOffering] full error object:', err);
          console.error(
      ...
      // DEBUG
      console.log(
        '[AddOffering] saveOfferingMedicalService payload:',
        medicalPayload,
      );
      console.log(
        '[AddOffering] saveOfferingMedicalService payload (JSON):',
        JSON.stringify(medicalPayload, null, 2),
      );

      this.businessService.saveOfferingMedicalService(medicalPayload).subscribe(
        (res) => {
          console.log(
            '[AddOffering] saveOfferingMedicalService response:',
            res,
          );
          this.finishSave();
        },
        (err) => {
          console.error('[AddOffering] saveOfferingMedicalService FAILED');
          console.error('[AddOffering] status:', err?.status, err?.statusText);
          console.error('[AddOffering] error body:', err?.error);
          console.error('[AddOffering] full error object:', err);
          console.error(
