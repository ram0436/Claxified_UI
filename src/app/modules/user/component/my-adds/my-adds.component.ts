...
export class MyAddsComponent {
  ...
  getCardImageURL(card: any): string {
    if (card.gadgetImageList && card.gadgetImageList[0]?.imageURL) {
      ...
      card.commercialServiceImagesList &&
      card.commercialServiceImagesList[0]?.imageURL
    ) {
  ...
  getCategoryName(ad: any): string {
    const mainCategory = this.mainCategories.find(
      (mainCategory: any) => mainCategory.id === ad.categoryId
    );
    if (mainCategory) {
  ...
  editAd(data: any) {
    ...
    let mainCategory = this.mainCategories.find(
      ...
      switch (mainCategory.categoryName) {
        case "Gadgets": {
          this.router.navigateByUrl(
            "/Gadgets/add-post?main=Gadgets&mode=edit&sub=Mobiles"
          );
          break;
        }
        case "Vehicles": {
          this.router.navigateByUrl(
            "/Vehicles/add-post?main=Vehicles&mode=edit&sub=Cars"
          );
          break;
        }
        case "Books": {
          this.router.navigateByUrl(
            "/Books/add-post?main=Books&mode=edit&sub=Science %26 Technology"
          );
          break;
        }
        case "Commercial Services": {
          this.router.navigateByUrl(
            "/Commercial Services/add-post?main=Commercial Services&mode=edit&sub=Finance %26 Management"
          );
          break;
        }
        case "Properties": {
          this.router.navigateByUrl(
            "/Properties/add-post?main=Properties&mode=edit&sub=For Sale: Houses %26 Apartments"
          );
          break;
        }
        case "Jobs": {
          this.router.navigateByUrl(
            "/Jobs/add-post?main=Jobs&mode=edit&sub=Data Entry %26 Back Office"
          );
          break;
        }
        case "Electronics & Appliances": {
          this.router.navigateByUrl(
            "/Electronics %26 Appliances/add-post?main=Electronics %26 Appliances&mode=edit&sub=TV"
          );
          break;
        }
        case "Furniture": {
          this.router.navigateByUrl(
            "/Furniture/add-post?main=Furniture&mode=edit&sub=Sofa %26 Dining"
          );
          break;
        }
        case "Sports & Hobbies": {
          this.router.navigateByUrl(
            "/Sports %26 Hobbies/add-post?main=Sports %26 Hobbies&mode=edit&sub=Gym %26 Fitness"
          );
          break;
        }
        case "Pets": {
          this.router.navigateByUrl(
            "/Pets/add-post?main=Pets&mode=edit&sub=Fishes %26 Aquarium"
          );
          break;
        }
        case "Fashion": {
          this.router.navigateByUrl(
            "/Fashion/add-post?main=Fashion&mode=edit&sub=Men"
          );
          break;
        }
      }
  ...
  onTabChanged(event: any) {
    if (event.index == 0) {
      this.activeAds = this.allAds.filter(
        (ad: any) => ad.isActive == true && ad.isVerified == true
      );
    } else if (event.index == 1)
      this.inActiveAds = this.allAds.filter(
        (ad: any) => ad.isActive == false && ad.isVerified == true
      );
    else
