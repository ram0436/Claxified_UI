...
export class AddPostComponent {
  ...
  constructor(private fashionService: FashionService, private commonService: CommonService, private snackBar: MatSnackBar, private route: ActivatedRoute, private AdminDashboardService: AdminDashboardService,
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if(this.mode !=undefined){
        ...
        this.fashionService.getFashionPostByGuid(guid).subscribe((res:any)=>{
          this.commonPayload = res[0];
          res[0].fashionImageList.forEach((image:any,index:any)=>{
  ...
  selectImage(event: any): void {
    ...
    this.fashionService.uploadFashionImages(formData).subscribe((data: any) => {
      this.progress = false;
  ...
  postAdd() {
    ...
    if(payload.id)
      this.updateFashionPost(payload);
    else
      this.saveFashionPost(payload);
  }
  ...
  saveFashionPost(payload: any) {
    if (this.validatePostForm(payload))
      this.fashionService.saveFashionPost(payload).subscribe(data => {
        this.showNotification("Post added succesfully");
        this.router.navigateByUrl('/post-menu');
      });
  }
  addSpecificPayload(commonPayload: any): any {
    var imageList: { fashionId: number; imageId: string; imageURL: any; }[] = [];
    this.cardsCount.forEach(imageURL => {
      if (imageURL != "")
        imageList.push({ "fashionId": 0, "imageId": "100", "imageURL": imageURL });
    });
    var payload = Object.assign({}, commonPayload, {
      fashionImageList: imageList,
    });
    return payload;
  }
  getUserData() {
  ...
  validatePostForm(payload: any): boolean {
    ...
    else if (payload.fashionImageList.length <= 0)
  ...
  updateFashionPost(payload: any) {
    if (this.validatePostForm(payload))
      this.fashionService.updateFashionPost(payload).subscribe(data => {
